...
const StudentRow = memo(({ student, onEdit, onDelete, onView, zebra }) => (
  <tr className={`border-b border-gray-100 last:border-0 hover:bg-indigo-50 transition-colors ${zebra ? 'bg-gray-50/60' : 'bg-white'}`}>
    ...
    <td className="px-4 py-3 flex gap-2">
      {onView && <button onClick={() => onView(student.id, getFullName(student))} className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 transition-colors">Attendance</button>}
