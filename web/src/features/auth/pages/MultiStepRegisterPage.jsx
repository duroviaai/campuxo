    ...
    [STEP_METHOD]: (
      <div className="space-y-2.5 pt-1">
        <button type="button" onClick={() => googleLogin()} disabled={googleLoading}
          className="w-full flex items-center gap-3 rounded-xl px-4 py-3.5 text-sm font-medium text-slate-700 transition-all disabled:opacity-60 hover:-translate-y-px active:scale-95"
          style={{ border: '1.5px solid #e2e8f0', background: '#fff', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = '#cbd5e1'; e.currentTarget.style.boxShadow = '0 4px 10px rgba(0,0,0,0.08)'; }}
          ...
          <span className="flex-1 text-left">
            <span className="block font-semibold text-slate-800">{googleLoading ? 'Connecting…' : 'Continue with Google'}</span>
            <span className="block text-xs text-slate-400 font-normal">Use your Google account</span>
          </span>
        </button>
