        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              value={selAssignment}
              onChange={(e) => { setSelAssignment(e.target.value); setRecords(null); }}
            >
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              value={date}
              onChange={(e) => { setDate(e.target.value); setRecords(null); }}
            />
          </div>
        </div>
        <button
  ...
  const handleLoadStudents = async () => {
    ...
    try {
      const [data, existing] = await Promise.all([
        getClassStudents(assignment.classId),
        axiosInstance
          .get(`/api/v1/attendance/course/${assignment.courseId}/class/${assignment.classId}`, {
            params: { date },
          })
      ...
      setStudents(data);
      const statusMap = {};
      data.forEach((s) => { statusMap[s.id] = 'PRESENT'; });
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              value={selAssignment}
              onChange={(e) => {
                setSelAssignment(e.target.value);
                setStudentsLoaded(false);
                setStudents([]);
                setAlreadySubmitted(false);
                setSubmitMsg(null);
                  ...
                  {students.map((s, i) => (
                    <tr key={s.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-3">
                        <button
                          onClick={() => navigate(
                            ROUTES.FACULTY_STUDENT_ATTENDANCE.replace(':studentId', s.id),
                            { state: { studentName: s.fullName || `${s.firstName} ${s.lastName}` } }
                          )}
                          className="text-xs text-indigo-600 hover:underline"
                        >
                          View Summary
    ...
    <div className="space-y-5 max-w-4xl mx-auto">
      ...
      {loadingAssignments ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-10 text-center space-y-2">
          ...
          <p className="text-xs text-gray-400">Contact your admin to get courses assigned to you.</p>
        </div>
      ) : tab === 'mark' ? (
